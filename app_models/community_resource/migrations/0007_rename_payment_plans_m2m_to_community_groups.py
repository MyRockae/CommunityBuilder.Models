# Renames implicit M2M through table Resource_payment_plans -> Resource_community_groups (data preserved).
...
def _rename_table_forward(schema_editor, old: str, new: str) -> None:
    ...
    if vendor == 'postgresql':
        ...
        raise NotImplementedError(
            f'Unsupported database vendor {vendor!r} for M2M table rename; extend 0007_rename_payment_plans_m2m_to_community_groups.'
        )


def forward(apps, schema_editor) -> None:
    _rename_table_forward(schema_editor, 'Resource_payment_plans', 'Resource_community_groups')


def backward(apps, schema_editor) -> None:
    _rename_table_forward(schema_editor, 'Resource_community_groups', 'Resource_payment_plans')
