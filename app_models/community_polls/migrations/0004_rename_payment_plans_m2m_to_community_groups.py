# Renames implicit M2M through table Poll_payment_plans -> Poll_community_groups (data preserved).
...
def forward(apps, schema_editor) -> None:
    _rename_table_forward(schema_editor, 'Poll_payment_plans', 'Poll_community_groups')


def backward(apps, schema_editor) -> None:
    _rename_table_forward(schema_editor, 'Poll_community_groups', 'Poll_payment_plans')
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('community_polls', '0003_rename_poll_payment_plans_m2m_column'),
    ]
