# Renames SimpleQuiz (or legacy Quiz) M2M through table to SimpleQuiz_community_groups; data preserved.
...
def forward(apps, schema_editor) -> None:
    # After RenameModel Quiz -> SimpleQuiz, through table may be Quiz_payment_plans or SimpleQuiz_payment_plans.
    if _table_exists(schema_editor, 'SimpleQuiz_payment_plans'):
        _rename_table_forward(schema_editor, 'SimpleQuiz_payment_plans', 'SimpleQuiz_community_groups')
    elif _table_exists(schema_editor, 'Quiz_payment_plans'):
        _rename_table_forward(schema_editor, 'Quiz_payment_plans', 'SimpleQuiz_community_groups')


def backward(apps, schema_editor) -> None:
    _rename_table_forward(schema_editor, 'SimpleQuiz_community_groups', 'SimpleQuiz_payment_plans')
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('community_quiz', '0004_rename_simple_quiz_tables_and_related_names'),
    ]
